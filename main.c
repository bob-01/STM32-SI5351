...
uint8_t position_x = 0;
  ...
  int main(void)
  {
    ...
    while (1) {
      ...
      if(error != HAL_OK) {
        print("SI5351 is ready!!!", 1, position_x, 0);
      ...
      si5351_set_freq(90600000UL, SI5351_CLK0);
      si5351_set_freq(101800000UL, SI5351_CLK2);

      uint8_t *x = si5351_read(16);
      print("SI5351 16 reg =", 1, position_x, 0);      
			printNumI(x , cfont.x_size*16, position_x, 1,' ');
      printNumI((uint8_t *)x , cfont.x_size*25, position_x, 1,' ');

      HAL_Delay(10000);

      setColor(255, 0, 0);
      print("CLK is OFF", 1, position_x += cfont.y_size, 0);      
      position_x += cfont.y_size;
